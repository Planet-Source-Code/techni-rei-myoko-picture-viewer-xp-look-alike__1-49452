...
Public Sub loadimage(Filename As String)
    ...
    If LCase(Filename) <> imagename And Filename <> Empty Then
        If ISaDIR(Filename) Then
            filemain.path = Filename
            If filemain.ListCount > 0 Then
                Filename = Replace(Filename & "\" & filemain.List(0), "\\", "\")
            Else
                Exit Sub
            End If
        End If
...
Private Sub Form_Load()
    ...
    WindowState = GetSetting(App.EXEName, "Main", "WindowState", WindowState)
    Width = GetSetting(App.EXEName, "Main", "Width", Width)
    Height = GetSetting(App.EXEName, "Main", "Height", Height)
    Top = GetSetting(App.EXEName, "Main", "Top", Top)
    Left = GetSetting(App.EXEName, "Main", "Left", Left)
...
Private Sub Form_Unload(Cancel As Integer)
    Call SaveSetting(App.EXEName, "Main", "WindowState", WindowState)
    WindowState = 0
    Call SaveSetting(App.EXEName, "Main", "Width", Width)
    Call SaveSetting(App.EXEName, "Main", "Height", Height)
    Call SaveSetting(App.EXEName, "Main", "Top", Top)
    Call SaveSetting(App.EXEName, "Main", "Left", Left)
End Sub
